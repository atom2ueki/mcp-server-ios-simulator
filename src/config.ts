...
// `quiet: true` suppresses dotenv v17+ default startup log on stdout, which
// would otherwise corrupt the MCP stdio JSON-RPC stream.
...
const envConfig = {
  simulator: {
    ...
    timeout: process.env.SIMULATOR_TIMEOUT
      ? parseInt(process.env.SIMULATOR_TIMEOUT, 10)
      : undefined,
  },
  mcp: {
